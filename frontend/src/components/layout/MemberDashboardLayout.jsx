...
import {
  ...
  MessageSquare, Bell, LogOut, Menu, X, Sparkles, Home, ChefHat
} from 'lucide-react';
...
export default function MemberDashboardLayout({ children, title }) {
      ...
      <aside className="flex max-lg:hidden flex-col w-64 h-screen sticky top-0 bg-surface/50 backdrop-blur-xl border-r border-white/10">
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3 p-3 glass rounded-xl mb-3">
            <div className="w-10 h-10 bg-gradient-primary rounded-full flex items-center justify-center font-bold text-white text-sm">
              {initials}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold truncate">{user?.full_name}</div>
              <div className="text-xs text-light-muted truncate">Member</div>
            </div>
          </div>
          <button
              ...
              <div className="p-6 border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-accent-400">Member</div>
                  </div>
                </div>
        ...
        <header className="sticky top-0 z-30 bg-surface/30 backdrop-blur-xl border-b border-white/10">
          <div className="h-16 flex items-center justify-between px-6">
            <div className="flex items-center gap-3">
              ...
              <h1 className="font-bold text-lg">{title || 'Dashboard'}</h1>
            </div>
